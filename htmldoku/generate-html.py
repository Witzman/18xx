...
def build_coverage_html():
    ...
    # Cross-links: coverage section name → (page, anchor) in docs
    # anchor = id attr as generated by convert_md (lower, spaces→dashes)
    _COV_RULES_LINK = {
        'Auction Phase':             ('rules-summary.html', 'auction-phase'),
        'Train Data & Phases':       ('rules-summary.html', 'train-phases--rusting'),
        'Stock Market Grid':         ('rules-summary.html', 'stock-market-movement'),
        'Stock Rounds':              ('rules-summary.html', 'stock-market-movement'),
        'Track Rights':              ('rules-summary.html', 'track-rights-zones'),
        'Track Rights Chit System':  ('rules-summary.html', 'track-rights-zones'),
        'OR Steps (Major)':          ('rules-summary.html', 'operating-round-per-company'),
        'Orient Express':            ('rules-summary.html', 'orient-express'),
        'Nationals':                 ('rules-summary.html', 'national-revenue-unique'),
        'Route & Revenue (Cross-Water)': ('rules-summary.html', 'cross-water-costs'),
        'Minor Abilities':           ('rules-summary.html', 'minor-abilities'),
        'Private Abilities':         ('rules-summary.html', 'private-abilities'),
        'Pullman Cars':              ('18oe-or-guide.html',  'pullman-cars'),
        'End Game':                  ('18oe-or-guide.html',  'end-game'),
        'Consolidation Phase':       ('18oe-or-guide.html',  'consolidation-phase'),
        'Railroad Formation':        ('18oe-player-guide.html', 'regional--major-conversion'),
    }

    # expand/collapse all buttons
    p.append('<div class="cov-expand-bar">')
    p.append('<button class="cov-expand-btn" id="cov-expand-all">Expand all</button>')
    p.append('<button class="cov-expand-btn" id="cov-collapse-all">Collapse all</button>')
    p.append('</div>')
    ...
    for chapter, items in COVERAGE_DATA:
        ...
        # per-section counts for progress bar + summary
        n_done    = sum(1 for _, s, _ in items if s == 'done')
        n_pr      = sum(1 for _, s, _ in items if s == 'needs-pr')
        n_partial = sum(1 for _, s, _ in items if s == 'partial')
        n_todo    = sum(1 for _, s, _ in items if s == 'todo')
        n_total   = len(items)
        t = n_total or 1

        def _pct(n): return f'{n / t * 100:.1f}%'

        # detail summary line: "5 done · 2 needs-PR · 1 partial · 2 todo"
        parts = []
        if n_done:    parts.append(f'<span class="cov-dr-done">{n_done} done</span>')
        if n_pr:      parts.append(f'<span class="cov-dr-pr">{n_pr} needs-PR</span>')
        if n_partial: parts.append(f'<span class="cov-dr-partial">{n_partial} partial</span>')
        if n_todo:    parts.append(f'<span class="cov-dr-todo">{n_todo} todo</span>')
        detail_summary = ' · '.join(parts)

        # cross-link for section title
        rules_ref = _COV_RULES_LINK.get(chapter)
        if rules_ref:
            rules_page, rules_anchor = rules_ref
            ch_name = (f'<a class="cov-rules-link" href="{rules_page}#{rules_anchor}"'
                       f' title="View in rules reference">{_html.escape(chapter)}</a>')
        else:
            ch_name = _html.escape(chapter)

        p.append(f'<div class="cov-chapter" data-chapter-scope="{chapter_scope}">')
        p.append(
            f'<div class="cov-chapter-title cov-toggle" id="{anchor}">'
            f'<span class="cov-ch-name">{ch_name}</span>'
            f'<div class="cov-sec-bar-wrap">'
            f'<div class="cov-sec-bar">'
            f'<div class="csb-seg csb-done"    style="width:{_pct(n_done)}"></div>'
            f'<div class="csb-seg csb-pr"      style="width:{_pct(n_pr)}"></div>'
            f'<div class="csb-seg csb-partial" style="width:{_pct(n_partial)}"></div>'
            f'</div></div>'
            f'<span class="cov-sec-count">{n_done + n_pr}/{n_total}</span>'
            f'</div>'
        )
        ...
        if detail_summary:
            p.append(f'<div class="cov-detail-summary">{detail_summary}</div>')
