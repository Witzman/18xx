  ...
  module Game
    module G18OE
      class Game < Game::Base
        ...
        def rust(train)
          super
          # Keep @depot as owner so unclaimed rusted trains group under @depot in
          # group_by(&:owner) and are swept into depot_trains by available.delete(@depot).
          # base.rb#rust sets owner=nil; nationals claim rusted trains via unclaimed_rusted_trains
          # which filters on owner == @depot rather than owner.nil?.
          train.owner = @depot
        end
